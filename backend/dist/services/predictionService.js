...
exports.getPredictions = exports.recalculatePredictions = void 0;
...
const recalculatePredictions = async () => {
    // Clear cache
    await (0, db_1.query)('TRUNCATE TABLE prediction_cache RESTART IDENTITY CASCADE');
    // We need distinct category/country pairs
    const pairs = await (0, db_1.query)('SELECT DISTINCT category, country FROM visa_bulletin_history');
    for (const row of pairs.rows) {
        const { category, country } = row;
        // Get last 6 months
        const history = await (0, db_1.query)(`
    ...
    `, [category, country]);
        ...
        if (retrogressions > 0)
            confidence = 'Low';
        else if (avgMonthlyMovementDays < 5)
            confidence = 'Medium';
...
exports.recalculatePredictions = recalculatePredictions;
const getPredictions = async () => {
    const res = await (0, db_1.query)('SELECT * FROM prediction_cache');
    return res.rows;
};
